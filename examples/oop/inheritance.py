...
# in all parent / super classes in the hierarchy untill it finds the method or
...
# %% md
# Above, ``identity`` attribute exists for ``Body`` and ``Spirity`` class but
# since ``Spirit`` class comes first, so ``NaN`` is printed which is value of
# ``identity`` attribute of ``Spirit`` class. Sometimes, we may want to initizlie
# both parent classes with separate initializing arguments. In following, ``Body``
# class requires two initizing arguments while ``Spirit`` class can be initialized
